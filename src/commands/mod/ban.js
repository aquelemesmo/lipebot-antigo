...
module.exports.run = async (bot,message,args,ops) => {
	if(!message.member.hasPermission(perms.ban)) return message.quote(msgs.semPerm)

	if(args.length === 0) {
		const length = new Discord.MessageEmbed()
		.setColor("PURPLE")
		.setTitle("Informações do comando **ban**")
		.addField("📜・Como usar?", "lp!ban <@usuário> <motivo>")
